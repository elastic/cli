...
`auto-loop` opts a PR into automatic patches. `skip-auto-loop` turns it off. Comment `/stop-repair` on the PR to halt and pin `skip-auto-loop`. Same-repo PRs only. At most two bot commits per PR.
