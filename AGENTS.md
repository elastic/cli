...
`auto-loop` opts a PR into automatic patches. `skip-auto-loop` turns it off. Comment `/stop-repair` on the PR to halt and pin `skip-auto-loop`. Comment `/review-no` on a bot review to dismiss it and store why in the `ai-review-memory` issue; the next AI review reads that. `skip-ai-review` silences the bot on that PR. Same-repo PRs only. At most two bot commits per PR.
