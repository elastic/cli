...
`auto-loop` opts a PR into automatic patches. `skip-auto-loop` turns it off. Comment `/stop-repair` on the PR to halt and pin `skip-auto-loop`. Comment `/bad` (optional reason) on the PR conversation to append a rejected bot finding to the `ai-review-memory` issue. A standing PR folds new comments into action items in `.github/skills/ai-review-memory.md` and records `<!-- processed-through: COMMENT_ID -->` on that file and the issue so already-folded comments are not read again. The AI review reads the skills file, not the issue thread. A top-level `/bad` dismisses the last bot review. `skip-ai-review` silences the bot on that PR. Same-repo PRs only. At most two bot commits per PR.
