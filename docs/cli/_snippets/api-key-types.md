...
- [{{es}} API keys](docs-content://deploy-manage/api-keys/elasticsearch-api-keys.md) for accessing {{es}} and {{kib}} APIs in stack-versioned deployments, including self-managed and {{ech}} deployments.
...
- [{{ecloud}} API keys](docs-content://deploy-manage/api-keys/elastic-cloud-api-keys.md) for managing organizations, {{ech}} deployments, and {{serverless-full}} projects. In Serverless, a Cloud API key with **Cloud, {{es}}, and {{kib}} API** access and an appropriate role for each relevant project can also call project-level {{es}} and {{kib}} APIs. {{ecloud}} API keys can't authenticate against {{es}} or {{kib}} endpoints on {{ech}}.
