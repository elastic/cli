...
**Why**: Loading every endpoint's schema at once allocates several gigabytes of heap (see `elastic/cli#171`). `loadEsApi()` / `loadEsApisInFile()` (and their `kb` equivalents) load only the namespace file for the endpoint actually invoked, memoized in a per-file module cache so repeated lookups in the same file are free. `src/lib/json-schema-refs.ts` then inlines each definition's sidecar `$ref`s (e.g. `_types.json`, shared by hundreds of definitions) into a self-contained schema, with sidecar files themselves loaded and expanded once and shared across every definition that references them.

Loading goes through `requireSchemaModule()` (`src/lib/json-schema-refs.ts`), a `createRequire()` wrapper, not `import.meta.resolve()` + dynamic `import()`: the standalone binaries built by `.github/workflows/release-artifacts.yml` (esbuild bundle + `pkg`) can resolve `require()` against the snapshot's `node_modules`, but not ESM URL resolution. Laziness is unchanged — `require()` is synchronous but still only runs for the namespace actually needed, and `scripts/perf-check` shows parity with the previous dynamic-`import()` form.
