...
describe('fetchWithHttpDebug', () => {
  it('delegates without writing output when debugging is disabled', async () => {
    ...
    const output = await captureProcessOutput(async () => {
      const result = await fetchWithHttpDebug(fakeFetch, 'https://example.test/data', { method: 'GET' })
  ...
  it('logs request and response details while redacting credential headers', async () => {
    ...
    let returnedBody = ''
    const output = await captureProcessOutput(async () => {
      const result = await fetchWithHttpDebug(fakeFetch, 'https://example.test/_search', {
        method: 'POST',
        headers: {
    ...
    assert.doesNotMatch(output.stderr, /request-secret|second-secret|cookie-secret|response-secret/)
  ...
  it('logs requests without bodies and responses with empty bodies', async () => {
    ...
    const { stderr } = await captureProcessOutput(async () => {
      await fetchWithHttpDebug(fakeFetch, 'https://example.test/empty', {
        method: 'HEAD',
  ...
  it('logs network failures and preserves the original error', async () => {
    ...
    const { stderr } = await captureProcessOutput(async () => {
      await assert.rejects(
        () => fetchWithHttpDebug(fakeFetch, 'https://example.test/fail', { method: 'GET' }),
  ...
  it('does not let an unreadable debug copy break the caller response', async () => {
    ...
    const { stderr } = await captureProcessOutput(async () => {
      const result = await fetchWithHttpDebug(fakeFetch, 'https://example.test/data', {
        method: 'GET',
