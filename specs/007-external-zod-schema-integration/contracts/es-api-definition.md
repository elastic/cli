...
2. `namespace` matches `/^[a-z][a-z-]*$/`
3. `path` starts with `/`
4. Every `{param}` token in `path` has a corresponding schema field with `found_in: "path"`
5. Every required schema field with `found_in: "path"` has a corresponding `{param}` token in `path`
6. No two schema keys produce the same kebab-case CLI flag after normalization
7. No CLI flag collides with reserved names: `help`, `version`, `format`, `config`, `context`, `file`
